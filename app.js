...
(() => {
  ...
  class Cloud {
    ...
    async setPhase(code,phase){
      if(!this.enabled) throw new Error('Firebase is not enabled.');
      // update() throws if the document doesn't exist, instead of merge-set's
      // behavior of silently creating a new sparse document. That throw is
      // exactly the signal we want if code/doc targeting is ever wrong.
      await this.gameDoc(code).update({
        phase,
        updatedAtMs:Date.now(),
        build:BUILD
      });
      // Read back directly from the server (not local cache) so we get proof
      // of what Firestore actually persisted, not just that our write call
      // resolved without throwing.
      const confirmSnap = await this.gameDoc(code).get({source:'server'});
      const confirmedPhase = confirmSnap.exists ? confirmSnap.data().phase : undefined;
      if(confirmedPhase !== phase){
        throw new Error(`Server confirms phase="${confirmedPhase}", expected "${phase}". Doc: iktGameStates/${code}`);
      }
  ...
  async function startGame(){
    ...
    lastSyncError='';
    console.log(`[IKT] startGame() firing. state.code="${state.code}" -> target doc iktGameStates/${state.code}`);
  ...
  function renderTVRoute(){
    ...
    console.log(`[IKT] TV subscribing to iktGameStates/${REQUESTED_CODE}`);
    cloud.subscribeGame(REQUESTED_CODE,(gameState)=>{
      console.log(`[IKT] TV snapshot received. phase="${gameState.phase}" updatedAtMs=${gameState.updatedAtMs}`);
      tvState=gameState;
