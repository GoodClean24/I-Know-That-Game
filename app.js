...
(() => {
  ...
  class Cloud {
    ...
    async save(code,state){
      ...
      await this.doc(code).set(clean,{merge:false});
    }
    async load(code){
    ...
    subscribe(code,cb){
      ...
      this.unsub = this.doc(code).onSnapshot(s => { if(s.exists) cb(s.data()); });
      return this.unsub;
    }
  ...
  function watchHostConnection(){
    ...
    connectWatcher = cloud.subscribe(state.code,data=>{
      if(data.displayLastSeenMs && data.displayLastSeenMs!==state.displayLastSeenMs){
        state.displayLastSeenMs=data.displayLastSeenMs;
        renderHost();
      }
    });
  }
  ...
  function renderTVRoute(){
    ...
    cloud.subscribe(REQUESTED_CODE,data=>{
      tvState=data;
